...
This stage requires you to create a variant of the position-lock focus-smoothing controller. The variation is that the center of the camera leads the target in the direction of the target movement. The position of the camera should move ahead of the target. This controller should update when movement input is given. When the target is not moving, the camera should not move. Should return to the target when the target has not moved for the `IdleDuration` and. The camera should not exceed `leadMaxDistance` from the target.
